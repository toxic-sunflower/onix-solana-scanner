...
### Mini App
- [ ] **Автологин в Mini App через `window.Telegram.WebApp.initData`** —
      backend-часть готова и рабочая: `POST /api/v1/auth/telegram-webapp`
      (`TelegramWebAppAuthValidator` — проверка HMAC-SHA256 подписи по
      официальному алгоритму Telegram Mini Apps). **Frontend-часть
      закоммичена, но временно ОТКЛЮЧЕНА** (закомментирована в
      `Landing.tsx`, скрипт `telegram-web-app.js` убран из `index.html`) —
      после первого деплоя мини-апп показывал пустой дашборд, обычный
      браузер при этом работал нормально. Не смог отладить вслепую на
      проде без реального `initData` от настоящей Telegram-сессии — нужно
      либо получить реальный `initData` для локальной отладки (DevTools в
      Telegram Desktop, или залогировать его один раз перед вызовом
      бэкенда), либо тестировать через реальный бот в тестовом окружении.
      Пока обычный OAuth-редирект остаётся единственным путём логина
      **везде**, включая Mini App — как было до этой попытки.
...
- [x] **Админка подключена к деплою (код/конфиг).** `src/Onix.Scanner.Admin/
      ...
      secrets (сделано вручную). Порт 5050 по-прежнему биндится только на
      `127.0.0.1` — наружу не торчит независимо от способа доступа.
- [x] **Админка доступна по `/admin` через nginx (код).** По просьбе —
      вместо SSH-туннеля/поддомена сделали путь на основном домене.
      `Program.cs` — `app.UsePathBase(pathBase)` из конфига `PathBase`
      (env `PathBase=/admin` в `docker-compose.yml`), должен идти самым
      первым в pipeline, до остальных `app.Use...`. `App.razor` —
      `<base href>` теперь читается из того же конфига вместо
      захардкоженного `/`. Поправлены все места, где были абсолютные пути
      (ломались бы под префиксом): `Login.razor` form action, `NavMenu.razor`
      logout-ссылка, `RedirectToLogin.razor` — везде убран ведущий `/`,
      теперь резолвятся относительно `<base href>`. Серверные редиректы в
      `Program.cs` (`/account/login`) теперь строятся как `$"{pathBase}/..."`
      вместо голого `"/..."`. nginx на этом же хосте (`network_mode: host`)
      достаёт `127.0.0.1:5050` напрямую — порт по-прежнему не открыт вовне,
      единственная публичная дверь — сам nginx. **Сам nginx location — см.
      "Критическое" выше, руками, я не могу.**
