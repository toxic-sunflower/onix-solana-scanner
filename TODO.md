...
- [ ] **Автологин в Mini App через `window.Telegram.WebApp.initData`** —
      backend готов: `POST /api/v1/auth/telegram-webapp`
      (`TelegramWebAppAuthValidator` — HMAC-SHA256 по официальному
      алгоритму Telegram Mini Apps). Frontend **включён повторно** после
      первого падения (пустой дашборд в мини-аппе, браузер при этом
      работал) — на этот раз с `console.log`/`console.error` на каждом шаге
      и 6-секундным таймаутом с автоматическим откатом на обычную кнопку
      логина, чтобы не зависать молча, как в первый раз. **Нужна проверка
      на реальном устройстве** — если снова сломается, в консоли
      DevTools (Telegram Desktop это позволяет) будет видно, на каком
      именно шаге и почему, вместо гадания вслепую.
